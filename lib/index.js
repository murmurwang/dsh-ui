...
// src/notes/files-service.ts
import { Remote as Remote2, TypertRemoteService as TypertRemoteService2 } from "@deepseek-ai/dsh-typert-protocol";
var _delete_dec2, _getBytes_dec, _get_dec2, _upload_dec, _list_dec2, _a2, _init2;
var FilesService = class extends (_a2 = TypertRemoteService2, _list_dec2 = [Remote2("list")], _upload_dec = [Remote2("upload")], _get_dec2 = [Remote2("get")], _getBytes_dec = [Remote2("getBytes")], _delete_dec2 = [Remote2("delete")], _a2) {
  constructor(ctx, store) {
    super(ctx, "files");
...
__decorateElement(_init2, 1, "list", _list_dec2, FilesService);
__decorateElement(_init2, 1, "upload", _upload_dec, FilesService);
__decorateElement(_init2, 1, "get", _get_dec2, FilesService);
__decorateElement(_init2, 1, "getBytes", _getBytes_dec, FilesService);
__decorateElement(_init2, 1, "delete", _delete_dec2, FilesService);
__decoratorMetadata(_init2, FilesService);

// src/notes/files-store.ts
import { mkdir as mkdir2, readFile as readFile2, readdir, rm, writeFile as writeFile2 } from "node:fs/promises";
import { randomUUID as randomUUID2 } from "node:crypto";
import { basename, join as join2 } from "node:path";
var FILE_MIME_WHITELIST = /* @__PURE__ */ new Set(["application/pdf"]);
...
var FilesStore = class {
  ...
  fileDir(id) {
    return join2(this.root, id);
  }
  async loadMeta(id) {
    try {
      const raw = await readFile2(join2(this.fileDir(id), "meta.json"), "utf8");
      const meta = JSON.parse(raw);
      const text = await readFile2(join2(this.fileDir(id), "text.txt"), "utf8").catch(() => void 0);
      return { ...meta, ...text !== void 0 ? { text } : {} };
    } catch {
      return null;
    }
  }
  ...
  upload(input) {
    return this.enqueue(async () => {
      ...
      if (!FILE_MIME_WHITELIST.has(input.mime)) {
        return { ok: false, error: err2(`unsupported mime "${input.mime}"\uFF08v1 \u4EC5\u652F\u6301 PDF\uFF09`) };
      }
      let bytes;
      try {
        bytes = Buffer.from(input.bytesBase64, "base64");
      } catch {
        return { ok: false, error: err2("invalid base64 payload") };
      }
      if (bytes.length === 0 || bytes.length > MAX_FILE_BYTES) {
        return { ok: false, error: err2(`file size ${bytes.length} out of range`) };
      }
      const id = randomUUID2();
      const dir = this.fileDir(id);
      await mkdir2(dir, { recursive: true });
      await writeFile2(join2(dir, "blob"), bytes);
      ...
      try {
        ...
        await writeFile2(join2(dir, "text.txt"), text, "utf8");
      } catch {
        text = void 0;
      }
      await writeFile2(join2(dir, "meta.json"), JSON.stringify(meta), "utf8");
      return { ok: true, value: { file: { ...meta, ...text !== void 0 ? { text } : {} } } };
    });
  }
  ...
  get(id) {
    return this.enqueue(async () => {
      ...
      if (meta === null) return { ok: false, error: notFound2(id) };
  ...
  getBytes(id) {
    return this.enqueue(async () => {
      ...
      const meta = await this.loadMeta(id);
      if (meta === null) return { ok: false, error: notFound2(id) };
      const bytes = await readFile2(join2(this.fileDir(id), "blob"));
...
function apply(ctx, config = {}) {
  ...
  const filesStore = new FilesStore(join3(root, "files"));
